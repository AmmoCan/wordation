...
// the bsProxy value should be the url to your project
// (i.e. 'local.dev', 'localhost:8888', or 'localhost/site1')
var bsProxy = '';
...
gulp.task('fonts', function() {
  return gulp.src(paths.fontPath + 'fontawesome-webfont.*')
    .pipe(gulp.dest(paths.destPath + 'fonts'))
    .pipe(notify({
			message: "✔︎ Font Awesome task complete",
			onLast: true
		}));
});
...
gulp.task('serve', ['svg-sprite', 'styles', 'fonts', 'js', 'browser-sync', 'foundation-js', 'watch']);

// Our default gulp task, which runs a one-time task
gulp.task('default', ['styles', 'fonts', 'js', 'svg-sprite']);
